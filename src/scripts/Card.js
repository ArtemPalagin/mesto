export class Card {
    constructor(cardData, selectores, handleCardClick, popupOpened, addLike, deliteLike) {
    ...
    _getTemplate() {
        ...
        if(this._ownerId == "9b4c619ac943e395e07e6a7c"){
    ...
    createPlaces() {
        ...
        for(let i = 0; i < this._likesNum; i++){
            if(this._likes[i]._id == "9b4c619ac943e395e07e6a7c"){
    ...
    _likeEventListeners() {
        this._element.querySelector(".element__group").addEventListener("click", (e) => {
            if(!e.target.classList.contains("element__group_black") ) {
                this._addLike(this._cardId, e.target.parentNode.querySelector(".element__group-number"));
            }else{
                this._deliteLike(this._cardId, e.target.parentNode.querySelector(".element__group-number"));
            }
